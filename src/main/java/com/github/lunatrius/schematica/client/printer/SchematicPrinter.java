...
public class SchematicPrinter {
        ...
        if (data != null) {
            switch (data.type) {
                ...
                case PISTON: {
                    Integer integer = data.mapping.get(ClientProxy.orientation);
                    if (integer != null) {
                        return BlockPistonBase.determineOrientation(null, x, y, z, player) == BlockPistonBase
                                .getPistonOrientation(metadata);
                    }
                    break;
                }
            }
    ...
    private boolean swapSlots(final int from, final int to) {
        return this.minecraft.playerController
                .windowClick(this.minecraft.thePlayer.inventoryContainer.windowId, from, to, 2,
